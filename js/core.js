...
/**
 * Tolerant triangulation for the batch view.
 * Unlike calculateTarget, this does NOT fail the whole group when a pair of
 * bearing lines is parallel or has no valid intersection — it skips that pair
 * and estimates from whatever valid pairwise intersections remain. Stations
 * that never produce a valid intersection are reported in `ignoredIds`.
 * Throws only when no valid intersection exists at all.
 * @returns {{
 *   target:{lat,lon},
 *   minAcuteAngle:{value:number, stationPair:[number,number]},
 *   usedIds:number[],
 *   ignoredIds:number[]
 * }}
 */
function calculateTargetTolerant(stations, options) {
  if (stations.length < 2) {
    throw new Error('至少需要 2 個觀測站');
  }

  const intersect = options.lineAlgorithm === 'geodesic' ? geodesicIntersect : planarIntersect;
  const validIntersections = [];
  const validPairAngles = [];
  ...
  for (let i = 0; i < stations.length; i++) {
    for (let j = i + 1; j < stations.length; j++) {
      const s1 = stations[i];
      const s2 = stations[j];
      if (acuteAngleBetween(s1.azimuth, s2.azimuth) < 0.5) continue; // parallel
      let pt;
      try {
        pt = intersect(s1, s2);
      } catch (e) {
        continue; // no valid (forward) intersection for this pair
      }
      validIntersections.push({ lat: pt.lat, lon: pt.lon });
      validPairAngles.push({ stations: [s1.id, s2.id], angle: acuteAngleBetween(s1.azimuth, s2.azimuth) });
      usedIds.add(s1.id);
      usedIds.add(s2.id);
    }
  }
  ...
  if (validIntersections.length === 0) {
    throw new Error('無有效交會（方位線平行或不相交）');
  }

  const usedStations = stations.filter(s => usedIds.has(s.id));
  const ignoredIds = stations.filter(s => !usedIds.has(s.id)).map(s => s.id);

  const centroid = centroidEstimate(validIntersections);
  const target = options.estimator === 'mle'
    ? mleEstimate(usedStations, centroid)
    : centroid;

  const minPair = validPairAngles.reduce((m, p) => p.angle < m.angle ? p : m);
